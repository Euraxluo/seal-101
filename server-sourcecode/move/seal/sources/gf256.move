// Copyright (c) Mysten Labs, Inc.
// SPDX-License-Identifier: Apache-2.0

/// GF(256)有限域实现模块
/// 
/// 该模块实现了GF(256)有限域上的数学运算，为阈值秘密共享等功能提供数学基础。
/// GF(256)是一个包含256个元素的有限域，在密码学中广泛应用于Reed-Solomon编码和秘密共享。
/// 
/// 主要功能包括:
/// - 有限域上的加法和减法（等价于异或运算）
/// - 有限域上的乘法和除法（通过指数和对数表实现）
/// - 指数和对数函数
module seal::gf256;


/// 指数表 - 将对数值映射到域元素
/// 指数表中的第i个元素是α^i，其中α是GF(256)的本原元
/// 注意：这个表中log值为255代表0（因为0没有对数）
const EXP: vector<u8> = vector[
    ,
];

/// 对数表 - 将域元素映射到对数值
/// 对数表中的第i个元素是log_α(i)，其中α是GF(256)的本原元
/// 注意：LOG[0]没有定义，我们用0表示（程序中需要特殊处理）
const LOG: vector<u8> = vector[
    ,
];

/// 计算指定元素的对数
/// 
/// 参数:
/// * x: 要计算对数的元素
#[allow(implicit_const_copy)]
fun log(x: u8): u16 {
    assert!(x != 0);
    *LOG.borrow((x - 1) as u64) as u16
}

/// 计算指数，给定一个对数值
/// 
/// 参数:
/// * x: 对数值
#[allow(implicit_const_copy)]
fun exp(x: u16): u8 {
    *EXP.borrow((x % 255) as u64)
}

/// 在GF(256)上执行加法（等价于异或运算）
/// 
/// 参数:
/// * x: A, y: B - 进行加法的两个元素
public(package) fun add(x: u8, y: u8): u8 {
    x ^ y
}

/// 在GF(256)上执行减法（在此域中，加法和减法是相同的，都是异或运算）
/// 
/// 参数:
/// * x: 被减数, y: 减数
public(package) fun sub(x: u8, y: u8): u8 {
    add(x, y)
}

/// 在GF(256)上执行乘法
/// 使用预计算的指数和对数表来实现高效的乘法
/// 
/// 参数:
/// * x: 第一个操作数, y: 第二个操作数
public(package) fun mul(x: u8, y: u8): u8 {
    if (x == 0 || y == 0) {
        return 0
    };
    exp(log(x) + log(y))
}

/// 在GF(256)上执行除法
/// 
/// 参数:
/// * x: 被除数, y: 除数
public(package) fun div(x: u8, y: u8): u8 {
    mul(x, exp(255 - log(y)))
}

#[test]
fun test_field_ops() {
    // Test vector, partly from https://en.wikipedia.org/wiki/Finite_field_arithmetic#Rijndael's_(AES)_finite_field
    let a = 0x53;
    let b = 0xca;
    assert!(add(a, b) == 0x99);
    assert!(sub(a, b) == 0x99);
    assert!(mul(a, b) == 0x01);
    assert!(div(a, b) == 0xb5);
}
