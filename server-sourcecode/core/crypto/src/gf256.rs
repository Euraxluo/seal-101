// Copyright (c), Mysten Labs, Inc.
// SPDX-License-Identifier: Apache-2.0

/**
 * GF(256)有限域实现模块
 * 
 * 本模块实现了GF(256)有限域，即伽罗瓦域（Galois Field）的256阶扩张域。
 * 特别是，这里实现的是Rijndael有限域，也是AES加密算法中使用的有限域。
 * 
 * GF(256)中的元素可以表示为8位二进制值，可以看作多项式F₂(x)模x⁸ + x⁴ + x³ + x + 1。
 * 例如，二进制值10011010可以看作多项式x⁷ + x⁴ + x³ + x。
 * 
 * 主要特点:
 * 1. 基于查表优化的乘法和除法操作
 * 2. 基于异或的加法和减法操作
 * 3. 支持所有基本域运算，包括乘法逆元
 * 4. 提供随机元素生成功能
 * 
 * 这种有限域的运算被广泛用于:
 * - 密码学（如AES和Reed-Solomon码）
 * - 秘密共享方案（如Shamir秘密共享）
 * - 纠错码（如前向纠错编码）
 */

use fastcrypto::error::FastCryptoResult;
use fastcrypto::{error::FastCryptoError::InvalidInput, traits::AllowedRng};
use rand::Rng;
use std::ops::{AddAssign, Neg};
use std::{
    iter::{Product, Sum},
    ops::{Add, Div, Mul, Sub},
};

/// 这表示256阶伽罗瓦域中的元素，表示为F₂(x) / <x⁸ + x⁴ + x³ + x + 1>，也称为Rinjdael有限域。
#[derive(Clone, Debug, Copy, Eq, PartialEq, Hash)]
pub struct GF256(pub(crate) u8);

/// 指数表 Eᵢ = gⁱ，其中g = 0x03是域乘法群的生成元。
/// 该表用于高效计算乘法：如果a = g^m，b = g^n，则a*b = g^(m+n)
const EXP: [u8; 255] = [
    ,
];

/// 对数表 Lᵢ = LOG[i + 1]，使得g^Lᵢ = i，其中g = 0x03。
/// 该表用于高效计算域元素的离散对数，便于乘法和除法操作
const LOG: [u8; 255] = [
    ,
];

/**
 * 计算GF256元素的离散对数
 * 
 * 返回非零元素x在有限域中的离散对数值
 * 即：如果x = g^y，则返回y
 * 
 * 参数:
 * @param x - 要计算对数的非零元素
 * 
 * 返回:
 * 元素的离散对数值
 * 
 * 注意：对于x=0，函数会panic，因为0没有对数
 */
fn log(x: &GF256) -> u16 {
    assert_ne!(x.0, 0);
    LOG[x.0 as usize - 1] as u16
}

/**
 * 计算GF256中的指数
 * 
 * 返回g^x，其中g=0x03是有限域的生成元
 * 
 * 参数:
 * @param x - 指数值
 * 
 * 返回:
 * 计算得到的GF256元素
 */
fn exp(x: u16) -> GF256 {
    GF256(EXP[x as usize % 255])
}

/**
 * GF256元素的加法实现
 * 
 * 在GF(256)中，加法是按位异或操作
 */
#[allow(clippy::suspicious_arithmetic_impl)]
impl Add<&GF256> for &GF256 {
    type Output = GF256;

    fn add(self, rhs: &GF256) -> Self::Output {
        GF256(self.0 ^ rhs.0)
    }
}

/**
 * GF256元素的加法赋值实现
 * 
 * 将当前元素与另一个元素相加（异或）并更新当前元素
 */
#[allow(clippy::suspicious_op_assign_impl)]
impl AddAssign<&GF256> for GF256 {
    fn add_assign(&mut self, rhs: &GF256) {
        self.0 ^= rhs.0;
    }
}

/**
 * GF256元素的减法实现
 * 
 * 在GF(256)中，减法与加法相同，都是按位异或操作
 */
#[allow(clippy::suspicious_arithmetic_impl)]
impl Sub<&GF256> for &GF256 {
    type Output = GF256;

    fn sub(self, rhs: &GF256) -> Self::Output {
        self + rhs // 在二进制域中，加法和减法操作相同
    }
}

/**
 * GF256元素的取负实现
 * 
 * 在GF(256)中，元素的负数等于元素本身
 */
impl Neg for &GF256 {
    type Output = GF256;

    fn neg(self) -> Self::Output {
        &GF256::zero() - self
    }
}

/**
 * GF256元素的乘法实现
 * 
 * 使用查表方法高效实现乘法：
 * 如果a = g^m，b = g^n，则a*b = g^(m+n)
 */
impl Mul<&GF256> for &GF256 {
    type Output = GF256;

    fn mul(self, rhs: &GF256) -> Self::Output {
        if self.0 == 0 || rhs.0 == 0 {
            GF256::zero()
        } else {
            exp(log(self) + log(rhs))
        }
    }
}

/**
 * GF256元素的除法实现
 * 
 * 使用查表方法高效实现除法：
 * 如果a = g^m，b = g^n，则a/b = g^(m-n)
 * 
 * 注意：除数不能为0，否则会返回错误
 */
#[allow(clippy::suspicious_arithmetic_impl)]
impl Div<&GF256> for &GF256 {
    type Output = FastCryptoResult<GF256>;

    fn div(self, rhs: &GF256) -> Self::Output {
        if rhs.0 == 0 {
            return Err(InvalidInput);
        } else if self.0 == 0 {
            return Ok(GF256::zero());
        }
        Ok(exp(255 + log(self) - log(rhs)))
    }
}

/**
 * 实现GF256元素的乘积聚合
 * 
 * 计算多个GF256元素的乘积
 */
impl Product for GF256 {
    fn product<I: Iterator<Item = Self>>(iter: I) -> Self {
        iter.fold(Self(1), |acc, x| &acc * &x)
    }
}

/**
 * 实现GF256元素的求和聚合
 * 
 * 计算多个GF256元素的和
 */
impl Sum for GF256 {
    fn sum<I: Iterator<Item = Self>>(iter: I) -> Self {
        iter.fold(Self(0), |acc, x| &acc + &x)
    }
}

/// 从u8类型转换为GF256类型
impl From<u8> for GF256 {
    fn from(value: u8) -> Self {
        Self(value)
    }
}

/// 从&u8类型转换为GF256类型
impl From<&u8> for GF256 {
    fn from(value: &u8) -> Self {
        Self(*value)
    }
}

/// 从GF256类型转换为u8类型
impl From<GF256> for u8 {
    fn from(value: GF256) -> Self {
        value.0
    }
}

impl GF256 {
    /**
     * 生成随机GF256元素
     * 
     * 使用随机数生成器创建随机的GF256元素
     * 
     * 参数:
     * @param rng - 随机数生成器
     * 
     * 返回:
     * 随机生成的GF256元素
     */
    pub fn rand<R: AllowedRng>(rng: &mut R) -> Self {
        Self(rng.gen())
    }

    /// 返回GF256域中的零元素（加法单位元）
    pub fn zero() -> Self {
        Self(0)
    }

    /// 返回GF256域中的单位元素（乘法单位元）
    pub fn one() -> Self {
        Self(1)
    }
}

#[cfg(test)]
mod tests {
    use crate::gf256::GF256;

    #[test]
    fn test_field_ops() {
        // Test vector, partly from https://en.wikipedia.org/wiki/Finite_field_arithmetic#Rijndael's_(AES)_finite_field
        let a = GF256(0x53);
        let b = GF256(0xca);
        assert_eq!(&a + &b, GF256(0x99));
        assert_eq!(&a - &b, GF256(0x99));
        assert_eq!(&a * &b, GF256(0x01));
        assert_eq!((&a / &b).unwrap(), GF256(0xb5));
    }
}
